 ...
 * commands).
 */
class DecoupledMotorEx @JvmOverloads constructor(cacheTolerance: Double, motorFactory: () -> DcMotorEx, encoderFactory: () -> DcMotorEx, name: String? = null) : Controllable {

    @JvmOverloads
    constructor(motorFactory: () -> DcMotorEx, encoderFactory: () -> DcMotorEx, name: String? = null) : this(0.01, motorFactory, encoderFactory, name)

    @JvmOverloads
    constructor(motor: DcMotorEx, encoder: DcMotorEx, cacheTolerance: Double = 0.01, name: String? = null) : this(cacheTolerance, { motor }, { encoder }, name)
    ...
    ) : this(cacheTolerance, { ActiveOpMode.hardwareMap[motorName] as DcMotorEx }, { ActiveOpMode.hardwareMap[encoderName] as DcMotorEx }, motorName)

    private val lazy = LazyHardware(motorFactory)
    val motor by lazy

    private val encoderLazy = LazyHardware(encoderFactory)
    val encoder by encoderLazy
     ...
     */
    val rawTicks: Double
        get() = encoder.currentPosition.toDouble()
    ...
    override val velocity: Double
        get() = encoder.velocity * direction
